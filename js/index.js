		        ...
		        },*/
		        {
		            ...
		            label: {
		                ...
		                formatter(params) {
		                    ...
		                    let arr = [params.name, "卫生预警：" + params.value[1] + "家", "操作规范预警：" + params.value[0] + "家"];
		                    return arr.join("\n")
		                },
		                textStyle: {
		                    align: 'left',
...
function wuran(){
	...
	var salvProName =["门店总数","优秀","良好","合格","需整改"];
	var salvProValue =[1759,1028,344,300,87];
...
function zhexian() {
	...
	dataObj = {
		...
		data: {
			value: [{
				name: '北京',
				value: [72, 88, 75, 81, 82, 86]
			}, {
				name: '上海',
				value: [67, 72, 76, 78, 87, 82]
			}, {
				name: '浙江',
				value: [68, 74, 72, 82, 80, 86]
			}, {
				name: '广东',
				value: [79, 73, 82, 86, 88, 83]
			}, {
				name: '重庆',
				value: [62, 75, 84, 82, 88, 81]
			}]
		}
	}
	
	let colors = ['#f0c725', '#16f892', '#0BE3FF', '#FF5733', '#A569BD']; // 为五条折线指定不同颜色
	let dataArr = []
	...
	dataObj.data.value.map((item, index) => {
		let newArr = {
			...
			itemStyle: {
				normal: {
					color: colors[index] // 根据 index 使用不同颜色
				}
			}
		}
		dataArr.push(newArr)
	...
	option = {
		color: colors, // 同时为 legend 和折线图同步颜色
		title: {
			left: 'center',
			text: '',
			textStyle: {
